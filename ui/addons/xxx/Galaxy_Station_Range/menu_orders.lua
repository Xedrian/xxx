...
function menu.cleanup()
	...
	menu.typestring = nil
	menu.owner = nil
	menu.blackboard_attackenemies = nil
	...
	if menu.rowDataMap[Helper.currentDefaultTableRow] then
		...
		if IsValidComponent(menu.entity) then
			...
			elseif rowdata == "subordinaterange" then
				...
				Helper.updateCellText(menu.selecttable, Helper.currentDefaultTableRow, 4, subordianterangecomponenttype .. ReadText(1001, 120) .. " " .. GetComponentData(menu.subordinaterangecomponent, "name"))
...
function menu.buttonComm()
	if IsValidComponent(menu.entity) then
		Helper.closeMenuForSubConversation(menu, false, "default", menu.entity, nil, (not Helper.useFullscreenDetailmonitor()) and "facecopilot" or nil)
	else
		Helper.closeMenuAndCancel(menu)
	end
	menu.cleanup()
...
function menu.buttonNewOrder()
	Helper.closeMenuForSubSection(menu, false, "gMain_charNewOrder", { 0, 0, menu.entity })
	menu.cleanup()
end

function menu.buttonExtend(cat, row)
	if menu.extendedcategories[cat] then
		menu.extendedcategories[cat] = nil
	else
		menu.extendedcategories[cat] = true
	end
	...
	if owner == "player" then
		...
		if hasownaccount then
			...
			elseif menu.typestring == "manager" then
				...
				Helper.setButtonScript(menu, nil, menu.selecttable, line, 1, function()
					return menu.buttonExtend("wantedmoney", line)
				end)
				nooflines = nooflines + 1
			end
		end
